...
const createPDF = async (orderCode: string, amount:string, title:string, nameCosFromQuery:string, nameDesFromQuery:string) => {
  ...
  page.drawText(`Bonjour ${nameDesFromQuery},${nameCosFromQuery} vous a offert une carte cadeau Karine-Beauté-Zen ! (${title})`, { x: 50, y: height - logoHeight - 100, size: 24 });
...
export const GET = async (request: NextRequest) => {
  try {
    ...
    const pdfBytes = await createPDF(orderCode, titleFromQuery, amountFromQuery, nameCosFromQuery, nameDesFromQuery);
    const pdfAttachment = {
    ...
    await transporter.sendMail({
      ...
      to: mailCosFromQuery || "karinebeautezen@gmail.com",
