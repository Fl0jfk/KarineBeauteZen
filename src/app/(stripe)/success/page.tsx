...
type CustomerData = { name: string; email: string; orderCode: string};

async function fetchCustomerData(params: Record<string, string | undefined>) {
...
export default function SuccessPage() {
  ...
  const namecos = searchParams.get('namecos') ?? undefined;
  const namedes = searchParams.get('namedes') ?? undefined;
  const mailcos = searchParams.get('mailcos') ?? undefined;
    ...
    <main className="w-full h-[70vh] flex items-center justify-center flex-col gap-3 text-center sm:top-[25vh] md:top-[25vh] p-4">
      ...
      <div>
        ...
        <ul>
          ...
          <li> Vous allez être redirigé vers la page d&apos;accueil dans 15 secondes !
          </li>
        </ul>
      </div>
    </main>
