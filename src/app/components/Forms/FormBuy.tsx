...
export default function FormBuy({ amount }: FormBuyProps) {
  ...
  const Checkout = async () => {
    ...
    try {
      ...
      if (formData.namedes) requestData.namedes = formData.namedes;
      if (formData.emaildes) requestData.emaildes = formData.emaildes;
      ...
      {isGift && ( 
        <>
          <div className="flex gap-4">
            ...
            <input id="namedes"  type="text"  placeholder="Nom et prénom"  className="w-full rounded-full border border-gray-300 px-6 outline-none focus:border-black-500 focus:shadow-md" {...register('namedes', { required: isGift })}/>
          </div>
          ...
          {giftOption === 'email' && (
            <div className="flex gap-4">
              ...
              <input  type="email"  id="emaildes"  placeholder="exemple@domaine.fr"  className="w-full rounded-full border border-gray-300 px-6 outline-none focus:border-black-500 focus:shadow-md" {...register('emaildes', { required: giftOption === 'email' })}/>
            </div>
          )}
        </>
      )}
      <button  type="button"  className={`bg-[#F2E9EB] p-2 rounded-md text-black ${!isValid ? "opacity-50 cursor-not-allowed" : ""}`}  onClick={Checkout}  disabled={!isValid || !giftOption}>Payer : {amount}€</button>
