...
export default function Card({ item }: CardProps) {
  ...
  return (
    <motion.div className="h-[400px] w-full" layout animate={{ opacity: 1, rotateY: isFlipped ? 180 : 360 }} transition={{ duration: 0.3 }} initial={false} exit={{ opacity: 0 }} onAnimationComplete={() => setIsAnimating(false)}>
      <div className={`h-[400px] rounded-xl overflow-hidden relative bg-white flip-card-front ${whatFace ? "" : "hidden"}`}>
        ...
        <div className="p-4">
          ...
          {item.title === "Carte cadeau" ? (
            <>
              ...
              <input
                ...
                className="bg-[#F2E9EB] p-2 rounded-md text-black w-full mb-2"
                min="10"
              />
            </>
          ) : (
          ...
          <button
            onClick={() => handleFlip()}
            className={`bg-[#F2E9EB] p-2 rounded-md text-black ${isButtonDisabled ? "opacity-50 cursor-not-allowed" : ""}`}
      ...
      <div
        className={`h-[400px] rounded-xl overflow-hidden flex flex-col gap-4 bg-white flip-card-back p-4 ${whatFace ? "hidden" : ""}`}
      >
